...
def get_repo_status(repo_name: Optional[str] = None) -> str:
    ...
    Args:
        repo_name: Optional specific repository to check
        
    Returns:
        Status string for the repository or overall status
    """
    try:
        if repo_name:
            ...
            return "Needs update" if "Your branch is behind" in output else "Up to date"
            
        # Check all repositories
        repos = get_lmt_repos_by_year()  # This now only returns existing repos
        total = sum(len(repos) for repos in repos.values())
        up_to_date = 0
        needs_update = 0
