...
#
#     if not password or not pid:
#         return 'Please enter both PID and password.', True, 'waring'
#
#     # If the user is authenticated, show a success message
#     if current_user.is_authenticated:
#         return 'You are already logged in.', True, 'info'
#     try:
#         # Fetch user and validate password
#         user = User.query.filter_by(username=pid).first()
...
#             login_user(user, remember=bool(remember_me))
#             if user.username == 'admin':
#                 return 'Login as Admin successful!', {'display','block'}
#             default_pid_path = f'{default_session_prefix}{user.username}'
#             [success, message] = pf.execute_git_pull(default_pid_path)
#             if not success:
#                 logger.error(f"Git pull failed for user {user.username}: {message}")
#                 return 'Git pull failed. Please check the logs for details.', True, 'danger'
#             return f'Login as {user.username} successful! {message}', True, 'success'
#         else:
#             return 'Invalid credentials. Please try again.', True, 'danger'
#
#     except Exception as e:
#         # Log the error
#         logger.error(f"Error during login for user {pid}: {str(e)}", exc_info=True)
#         return 'An error occurred. Please try again later.', True, 'danger'
...
# )
# def login_state(n_clicks, pid, password, is_open, data, remember_me):
#     """Handle user login."""
#     if not n_clicks:
...
#             login_user(user, remember=bool(remember_me))
#             default_pid_path = f'{default_session_prefix}{user.username}'
#             print('default_pid_path:', default_pid_path)
#             pf.execute_git_pull(default_pid_path)
#             # Log successful login
#             log_login_attempt(
#                 username=pid,
#                 success=True,
...
#
#             if user.username == 'admin':
#                 return f'{prefix}admin', 'Login successful!', True, 'success', data, ''
#
#             return f'{prefix}project/{pid}', 'Login successful!', False, '', data, ''
#
#         else:
#             # Log failed login attempt
#             log_login_attempt(
